...
public class RenderAdvancedBlockInterface extends RenderBlockInterface {

    public RenderAdvancedBlockInterface() {
    }
    ...
    public boolean renderInWorld(BlockInterface block, IBlockAccess world, int x, int y, int z, RenderBlocks renderer) {
        ...
        if (ti != null && ti.getForward() != ForgeDirection.UNKNOWN) {
            ...
            if (block instanceof BlockAdvancedInterface) {
                BlockAdvancedInterface blockInterface = (BlockAdvancedInterface) block;
