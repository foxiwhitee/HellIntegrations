...
public abstract class UniversalMultiTypePatternHelper implements ICraftingPatternDetailsExt, Comparable<UniversalMultiTypePatternHelper> {
    ...
    private final Set<UniversalMultiTypePatternHelper.TestLookup> failCache = new HashSet();
    private final Set<UniversalMultiTypePatternHelper.TestLookup> passCache = new HashSet();
    private final IAEItemStack pattern;
    private int priority = 0;
    private String type = "";


    public UniversalMultiTypePatternHelper(ItemStack is, World w) {
        ...
        if (encodedValue == null) {
            ...
            type = encodedValue.getString("typeCraft");
            this.patternItem = is;
            ...
            this.standardRecipe = findMatchingRecipeIn(this.crafting, this.type);
    ...
    private IHellRecipe findMatchingRecipeIn(InventoryCrafting matrix, String type) {
        if(this instanceof ManaPoolPatternHelper) {
            return ManaPoolPatternHelper.findMatchingRecipe(matrix, type);
        }
        return null;
    }
    ...
    private void markItemAs(int slotIndex, ItemStack i, TestStatus b) {
        if (b != TestStatus.TEST && !i.hasTagCompound()) {
            (b == TestStatus.ACCEPT ? this.passCache : this.failCache).add(new UniversalMultiTypePatternHelper.TestLookup(slotIndex, i));
        }
    }
    ...
    private TestStatus getStatus(int slotIndex, ItemStack i) {
        ...
        } else if (this.passCache.contains(new UniversalMultiTypePatternHelper.TestLookup(slotIndex, i))) {
            return TestStatus.ACCEPT;
        } else {
            return this.failCache.contains(new UniversalMultiTypePatternHelper.TestLookup(slotIndex, i)) ? TestStatus.DECLINE : TestStatus.TEST;
        }
    }

    public int compareTo(UniversalMultiTypePatternHelper o) {
    ...
    public boolean equals(Object obj) {
        if (obj == null) {
            ...
            UniversalMultiTypePatternHelper other = (UniversalMultiTypePatternHelper)obj;
    ...
    private static final class TestLookup {
        ...
        public boolean equals(Object obj) {
            boolean equality;
            if (obj instanceof UniversalMultiTypePatternHelper.TestLookup) {
                UniversalMultiTypePatternHelper.TestLookup b = (UniversalMultiTypePatternHelper.TestLookup)obj;
