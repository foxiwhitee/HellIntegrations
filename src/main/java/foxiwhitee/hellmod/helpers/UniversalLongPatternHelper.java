...
public abstract class UniversalLongPatternHelper implements ICraftingPatternDetails, Comparable<UniversalLongPatternHelper> {
    ...
    private final Set<UniversalLongPatternHelper.TestLookup> failCache = new HashSet();
    private final Set<UniversalLongPatternHelper.TestLookup> passCache = new HashSet();
    private final IAEItemStack pattern;
    private int priority = 0;


    public UniversalLongPatternHelper(ItemStack is, World w) {
        ...
        if (encodedValue == null) {
            ...
            NBTTagCompound inTag = encodedValue.getCompoundTag("in");
            NBTTagCompound outTag = encodedValue.getCompoundTag("out");
            this.patternItem = is;
            this.pattern = AEItemStack.create(is);
            List<IAEItemStack> in = new ArrayList();
            List<IAEItemStack> out = new ArrayList();

            ItemStack gs = ItemStack.loadItemStackFromNBT(inTag.getCompoundTag("0"));
            if (gs != null) {
                gs.stackSize = (int) inTag.getCompoundTag("0").getLong("Cnt");
            }
            this.crafting.setInventorySlotContents(0, gs);
            if (gs != null && (!this.isCrafting || !gs.hasTagCompound())) {
                this.markItemAs(0, gs, TestStatus.ACCEPT);
            }

            in.add(AEApi.instance().storage().createItemStack(gs));
            gs = ItemStack.loadItemStackFromNBT(outTag.getCompoundTag("0"));
            if (gs != null) {
                gs.stackSize = (int) outTag.getCompoundTag("0").getLong("Cnt");
            }
            this.testFrame.setInventorySlotContents(0, gs);
            if (gs != null) {
    ...
    private IHellRecipe findMatchingRecipeIn(InventoryCrafting matrix) {
        if(this instanceof NeutronPatternHelper) {
            return NeutronPatternHelper.findMatchingRecipe(matrix);
        }
        return null;
    }
    ...
    private void markItemAs(int slotIndex, ItemStack i, TestStatus b) {
        if (b != TestStatus.TEST && !i.hasTagCompound()) {
            (b == TestStatus.ACCEPT ? this.passCache : this.failCache).add(new UniversalLongPatternHelper.TestLookup(slotIndex, i));
        }
    }
    ...
    private TestStatus getStatus(int slotIndex, ItemStack i) {
        ...
        } else if (this.passCache.contains(new UniversalLongPatternHelper.TestLookup(slotIndex, i))) {
            return TestStatus.ACCEPT;
        } else {
            return this.failCache.contains(new UniversalLongPatternHelper.TestLookup(slotIndex, i)) ? TestStatus.DECLINE : TestStatus.TEST;
        }
    }

    public int compareTo(UniversalLongPatternHelper o) {
    ...
    public boolean equals(Object obj) {
        if (obj == null) {
            ...
            UniversalLongPatternHelper other = (UniversalLongPatternHelper)obj;
    ...
    private static final class TestLookup {
        ...
        public boolean equals(Object obj) {
            boolean equality;
            if (obj instanceof UniversalLongPatternHelper.TestLookup) {
                UniversalLongPatternHelper.TestLookup b = (UniversalLongPatternHelper.TestLookup)obj;
