...
public abstract class UniversalMultiPatternHelper  implements ICraftingPatternDetails, Comparable<UniversalMultiPatternHelper> {
    ...
    protected final List<IHellRecipe> standardRecipes;
    protected IHellRecipe standardRecipe;
    ...
    public UniversalMultiPatternHelper(ItemStack is, World w) {
        ...
        if (encodedValue == null) {
            ...
            this.standardRecipes = findMatchingRecipeIn(this.crafting);
            this.standardRecipe = this.standardRecipes.get(0);
    ...
    private List<IHellRecipe> findMatchingRecipeIn(InventoryCrafting matrix) {
        if (this instanceof AlchemicalConstructionPatternHelper) {
            return AlchemicalConstructionPatternHelper.findMatchingRecipe(matrix);
        }
        return null;
    }
    ...
    private TestStatus getStatus(int slotIndex, ItemStack i) {
        ...
        } else if (this.passCache.contains(new TestLookup(slotIndex, i))) {
            return TestStatus.ACCEPT;
        } else {
            return this.failCache.contains(new TestLookup(slotIndex, i)) ? TestStatus.DECLINE : TestStatus.TEST;
        }
    }

    public int compareTo(UniversalMultiPatternHelper o) {
    ...
    public boolean equals(Object obj) {
        if (obj == null) {
            ...
            UniversalMultiPatternHelper other = (UniversalMultiPatternHelper)obj;
