...
public class RenderChaoticBowModel implements IItemRenderer {
    ...
    boolean isDraconic = true;

    private ResourceLocation model;

    public RenderChaoticBowModel() {
        for (int i = 0; i < 4; i++)
            this.draconicModels[i] = AdvancedModelLoader.loadModel(DraconicEvolutionIntegration.getResource("models/tools/ChaoticBow0" + i + ".obj"));
    ...
    public void renderItem(IItemRenderer.ItemRenderType type, ItemStack item, Object... data) {
        ...
        if (this.isDraconic) {
            ...
            DraconicEvolutionIntegration.bindResource("textures/models/tools/ChaoticBow0" + selection + ".png");
        } else {
            activeModel = this.wyvernModels[selection];
        }
        if (activeModel != null)
    ...
    private void doRender(IModelCustom modelCustom, IItemRenderer.ItemRenderType type, int drawState, BowHandler.BowProperties properties) {
        ...
        if (drawState != -1) {
            GL11.glTranslated(0.3D, 0.151D, -0.2D + ((drawState == 1) ? 0.0D : ((drawState == 2) ? 0.55D : ((drawState == 3) ? 1.0D : -0.7D))));
