...
import foxiwhitee.hellmod.integration.draconic.blocks.BlockChaos;
...
public class EntityArialHeart extends Entity {
    ...
    public EntityArialHeart(World world) {
        super(world);
        this.setSize(0.25F, 0.25F);
    }

    public EntityArialHeart(World par1World, double x, double y, double z) {
        super(par1World);
            ...
            case 2: {
                ...
                for (EntityItem item : items) {
                    ...
                    if (Utills.getDistanceAtoB(posX, posY + 0.5, posZ, item.posX, item.posY, item.posZ) < 1) {
                        if (coresConsumed == HellConfig.coresNeedsForArial || worldObj.isRemote) break;
                        if (stack.getItem() != ModItems.chaoticCore) {
                            item.motionX = 1;
                            item.motionY = 6;
                            item.motionZ = 1;
                            continue;
                        }
                        int needed = HellConfig.coresNeedsForArial - coresConsumed;
                        if (stack.stackSize >= needed) {
                            coresConsumed = HellConfig.coresNeedsForArial;
                    ...
                    for (int x = (int) posX - 5; x <= (int) posX + 5; x++) {
                        for (int y = (int) posY - 5; y <= (int) posY + 5; y++) {
                            for (int z = (int) posZ - 5; z <= (int) posZ + 5; z++) {
                                if (worldObj.getBlock(x, y, z) instanceof BlockChaos && worldObj.getBlockMetadata(x, y, z) == 1) {
