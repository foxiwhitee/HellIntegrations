...
import foxiwhitee.hellmod.integration.draconic.client.render.armor.ModelChaoticArmor;
...
public class ChaoticArmor extends ItemArmor implements ISpecialArmor, IConfigurableItem, IInventoryTool, IGoggles, IRevealer, ICustomArmor, ICustomUpgradableItem {
    ...
    public ChaoticArmor(ItemArmor.ArmorMaterial armorMaterial, int armorType, String name) {
        super(armorMaterial, 0, armorType);
        this.maxEnergy = HellConfig.chaoticArmorBaseStorage;
        this.maxTransfer = HellConfig.chaoticArmorMaxTransfer;
    ...
    public void registerIcons(IIconRegister iconRegister) {
        this.helmIcon = iconRegister.registerIcon(HellCore.MODID + ":chaotic_helmet");
        this.chestIcon = iconRegister.registerIcon(HellCore.MODID + ":chaotic_chestplate");
        this.leggsIcon = iconRegister.registerIcon(HellCore.MODID + ":chaotic_leggings");
        this.bootsIcon = iconRegister.registerIcon(HellCore.MODID + ":chaotic_boots");
    }
    ...
    public ISpecialArmor.ArmorProperties getProperties(EntityLivingBase player, ItemStack armor, DamageSource source, double damage, int slot) {
        return (!source.isUnblockable() && !source.isDamageAbsolute() && !source.isMagicDamage()) ? new ISpecialArmor.ArmorProperties(0, this.damageReduceAmount / 24.5D, 1000) : new ISpecialArmor.ArmorProperties(0, this.damageReduceAmount / 100.0D, 15);
    }
        ...
        if (stack != null) {
            if (stack.getItem() == DraconicEvolutionIntegration.chaotic_helm) {
                ...
                if (getEnergyStored(stack) >= HellConfig.chaoticArmorEnergyToRemoveEffects && clearNegativeEffects((Entity)player))
                    extractEnergy(stack, HellConfig.chaoticArmorEnergyToRemoveEffects, false);
                ...
                if (player.getFoodStats().getFoodLevel() < 20 && IConfigurableItem.ProfileHelper.getBoolean(stack, "Saturation", false)) {
    ...
    public int getMaxEnergyStored(ItemStack container) {
        ...
        return HellConfig.chaoticArmorBaseStorage + points * HellConfig.chaoticArmorStoragePerUpgrade;
    }
    ...
    public ModelBiped getArmorModel(EntityLivingBase entityLiving, ItemStack itemStack, int armorSlot) {
        ...
        if (this.model == null) {
            if (this.armorType == 0) {
                this.model = (ModelBiped)new ModelChaoticArmor(1.1F, true, false, false, false);
            } else if (this.armorType == 1) {
                this.model = (ModelBiped)new ModelChaoticArmor(1.1F, false, true, false, false);
            } else if (this.armorType == 2) {
                this.model = (ModelBiped)new ModelChaoticArmor(1.1F, false, false, true, false);
            } else {
                this.model = (ModelBiped)new ModelChaoticArmor(1.1F, false, false, false, true);
            }
        ...
        return new ArrayList<ICustomUpgradableItem.EnumUpgrade>() {
            {
                ...
                this.add(ICustomUpgradableItem.EnumUpgrade.SHIELD_CAPACITY);
                this.add(ICustomUpgradableItem.EnumUpgrade.SHIELD_RECOVERY);
            }
        };
    }
    ...
    public int getUpgradeCap(ItemStack itemstack) {
        return HellConfig.chaoticArmorMaxUpgrades;
    }
    ...
    public int getMaxUpgradePoints(int upgradeIndex) {
        if (upgradeIndex == ICustomUpgradableItem.EnumUpgrade.RF_CAPACITY.index)
            return 60;
        if (upgradeIndex == ICustomUpgradableItem.EnumUpgrade.SHIELD_CAPACITY.index)
            return 200;
        return 164;
    }
    ...
    public int getBaseUpgradePoints(int upgradeIndex) {
        ...
        return (upgradeIndex == ICustomUpgradableItem.EnumUpgrade.SHIELD_RECOVERY.index) ? 20 : 0;
    }

    public float getProtectionPoints(ItemStack stack) {
        return (ICustomUpgradableItem.EnumUpgrade.SHIELD_CAPACITY.getUpgradePoints(stack) * 15);
    }

    public int getRecoveryPoints(ItemStack stack) {
        return ICustomUpgradableItem.EnumUpgrade.SHIELD_RECOVERY.getUpgradePoints(stack);
    }
    ...
    public float getJumpModifier(ItemStack stack, EntityPlayer player) {
        ...
        return (!player.isSprinting() && !BrandonsCore.proxy.isCtrlDown()) ? (IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorJumpMult", 0.0F) / 5.0F) : IConfigurableItem.ProfileHelper.getFloat(stack, "ArmorJumpMult", 0.0F);
    }
    ...
    public int getEnergyPerProtectionPoint() {
        return HellConfig.chaoticArmorEnergyPerProtectionPoint;
    }
