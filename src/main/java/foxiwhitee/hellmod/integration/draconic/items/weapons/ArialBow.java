...
public class ArialBow extends ItemBow implements IInventoryTool, IEnergyContainerWeaponItem, IHudDisplayItem, ICustomUpgradableItem {
    ...
    public List<ItemConfigField> getFields(ItemStack stack, int slot) {
        ...
        list.add((new ItemConfigField(4, slot, "BowArrowDamage")).setMinMaxAndIncromente(Float.valueOf(getBaseUpgradePoints(EnumUpgrade.ARROW_DAMAGE.index)), Float.valueOf(EnumUpgrade.ARROW_DAMAGE.getUpgradePoints(stack)), Float.valueOf(0.1F)).readFromItem(stack, Float.valueOf(EnumUpgrade.ARROW_DAMAGE.getUpgradePoints(stack))));
        list.add((new ItemConfigField(4, slot, "BowArrowSpeedModifier")).setMinMaxAndIncromente(Float.valueOf(0.0F), Float.valueOf(EnumUpgrade.ARROW_SPEED.getUpgradePoints(stack)), Float.valueOf(0.01F)).readFromItem(stack, Float.valueOf(0.0F)).setModifier("PLUSPERCENT"));
        ...
        return new ArrayList<EnumUpgrade>() {
            {
                ...
                this.add(EnumUpgrade.DRAW_SPEED);
                this.add(EnumUpgrade.ARROW_SPEED);
                this.add(EnumUpgrade.ARROW_DAMAGE);
            }
        };
    }
    ...
    public int getUpgradeCap(ItemStack itemstack) {
        return HellConfig.arialBowMaxUpgrades;
    }
    ...
    public int getMaxUpgradePoints(int upgradeIndex) {
        if (upgradeIndex == EnumUpgrade.RF_CAPACITY.index)
            return HellConfig.arialBowMaxCapacityUpgradePoints;
        if (upgradeIndex == EnumUpgrade.DRAW_SPEED.index)
            return 16;
        if (upgradeIndex == EnumUpgrade.ARROW_SPEED.index)
            return 40;
        return (upgradeIndex == EnumUpgrade.ARROW_DAMAGE.index) ? 60 : HellConfig.arialBowMaxUpgradePoints;
    }
    ...
    public int getBaseUpgradePoints(int upgradeIndex) {
        ...
        if (upgradeIndex == EnumUpgrade.ARROW_SPEED.index)
            return 10;
        return (upgradeIndex == EnumUpgrade.ARROW_DAMAGE.index) ? 6 : 0;
    }
    ...
    public List<String> getUpgradeStats(ItemStack stack) {
        ...
        list.add(InfoHelper.ITC() + LocalizationUtils.localize("gui.de.max.txt") + " " + LocalizationUtils.localize("gui.de.ArrowSpeed.txt") + ": " + InfoHelper.HITC() + "+" + (EnumUpgrade.ARROW_SPEED.getUpgradePoints(stack) * 100) + "%");
        list.add(InfoHelper.ITC() + LocalizationUtils.localize("gui.de.max.txt") + " " + LocalizationUtils.localize("gui.de.ArrowDamage.txt") + ": " + InfoHelper.HITC() + EnumUpgrade.ARROW_DAMAGE.getUpgradePoints(stack) + "");
    ...
    public int getMaxEnergyStored(ItemStack stack) {
        ...
        return HellConfig.arialWeaponsBaseStorage + points * HellConfig.arialWeaponsStoragePerUpgrade;
    }
    ...
    public int getEnergyPerAttack() {
        return HellConfig.arialBowEnergyPerShot;
    }
}
