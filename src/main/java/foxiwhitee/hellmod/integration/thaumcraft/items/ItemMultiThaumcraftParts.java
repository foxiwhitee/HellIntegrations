...
import foxiwhitee.hellmod.integration.thaumcraft.parts.EnumPartsThaumcraftTerminals;
...
public class ItemMultiThaumcraftParts extends AEBaseItem implements IPartItem, IItemGroup {
    private static final int INITIAL_REGISTERED_CAPACITY = EnumPartsThaumcraftTerminals.values().length;
    private static final Comparator<Map.Entry<Integer, EnumPartsWithVariant>> REGISTERED_COMPARATOR = new RegisteredComparator();
    public static ItemMultiThaumcraftParts instance;
    private final NameResolver nameResolver;
    private final Map<Integer, EnumPartsWithVariant> registered;
    public ItemMultiThaumcraftParts(IPartHelper partHelper) {
        Preconditions.checkNotNull(partHelper);
        this.setUnlocalizedName("thaumncraftPart");
        ...
        for(EnumPartsThaumcraftTerminals part : EnumPartsThaumcraftTerminals.values()) {
            if (part != EnumPartsThaumcraftTerminals.InvalidTypeThaumcraft) {
                if (part.isCable()) {
    ...
    @Nonnull
    public ItemStackSrc createPart(EnumPartsThaumcraftTerminals mat) {
        Preconditions.checkNotNull(mat);
        return this.createPart(mat, 0);
    }

    @Nonnull
    public ItemStackSrc createPart(EnumPartsThaumcraftTerminals mat, AEColor color) {
        Preconditions.checkNotNull(mat);
    ...
    @Nonnull
    private ItemStackSrc createPart(EnumPartsThaumcraftTerminals mat, int varID) {
        assert mat != null;
    ...
    private void processMetaOverlap(boolean enabled, int partDamage, EnumPartsThaumcraftTerminals mat, EnumPartsWithVariant pti) {
        assert partDamage >= 0;
    ...
    public int getDamageByType(EnumPartsThaumcraftTerminals t) {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World w, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        return this.getTypeByStack(is) == EnumPartsThaumcraftTerminals.InvalidTypeThaumcraft ? false : AEApi.instance().partHelper().placeBus(is, x, y, z, side, player, w);
    }
    ...
    public String getItemStackDisplayName(ItemStack is) {
        EnumPartsThaumcraftTerminals pt = this.getTypeByStack(is);
        if (pt.isCable()) {
    ...
    private String getName(ItemStack is) {
        ...
        EnumPartsThaumcraftTerminals stackType = this.getTypeByStack(is);
        String typeName = stackType.name();
        return typeName;
    }

    @Nonnull
    public EnumPartsThaumcraftTerminals getTypeByStack(ItemStack is) {
        Preconditions.checkNotNull(is);
        EnumPartsWithVariant pt = (EnumPartsWithVariant)this.getRegistered().get(is.getItemDamage());
        return pt != null ? pt.part : EnumPartsThaumcraftTerminals.InvalidTypeThaumcraft;
    }

    @Nullable
    public IPart createPartFromItemStack(ItemStack is) {
        EnumPartsThaumcraftTerminals type = this.getTypeByStack(is);
    ...
    protected static final class EnumPartsWithVariant {
        private final EnumPartsThaumcraftTerminals part;
        private final int variant;
        @SideOnly(Side.CLIENT)
        private IIcon ico;

        private EnumPartsWithVariant(EnumPartsThaumcraftTerminals part, int variant) {
            assert part != null;
