...
@SimpleGuiHandler(index = GuiHandlers.neutronUnifier, tile = TileNeutronUnifier.class, container = ContainerNeutronUnifier.class, gui = GuiNeutronUnifier.class, integration = "Avaritia")
public class BlockNeutronUnifier extends AEBaseTileBlock {
    protected final Random randy;

    public BlockNeutronUnifier(String name) {
        super(Material.iron);
        this.randy = new Random();
        setHardness(10.0F);
        setBlockTextureName(HellCore.MODID + ":ae2/" + name);
        setBlockName("name");
        setTileEntity(TileNeutronUnifier.class);
        this.isOpaque = false;
        this.lightOpacity = 1;
    }
    ...
    public boolean onActivated(World world, int x, int y, int z, EntityPlayer player, int p_149727_6_, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        ...
        if (tile instanceof TileNeutronUnifier && !world.isRemote)
            player.openGui(HellCore.instance, GuiHandlers.neutronUnifier, world, x, y, z);
        return true;
    }

    public void breakBlock(World world, int x, int y, int z, Block block, int wut) {
        TileNeutronUnifier unifier = (TileNeutronUnifier)world.getTileEntity(x, y, z);
        if (unifier != null) {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack itemstack) {
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile instanceof TileNeutronUnifier) {
            TileNeutronUnifier te = (TileNeutronUnifier)tile;
