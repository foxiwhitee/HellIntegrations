...
import foxiwhitee.hellmod.integration.botania.parts.EnumPartsBotaniaTerminals;
...
public class ItemMultiBotaniaParts extends AEBaseItem implements IPartItem, IItemGroup {
    private static final int INITIAL_REGISTERED_CAPACITY = EnumPartsBotaniaTerminals.values().length;
    private static final Comparator<Map.Entry<Integer, EnumPartsWithVariant>> REGISTERED_COMPARATOR = new RegisteredComparator();
    public static ItemMultiBotaniaParts instance;
    private final NameResolver nameResolver;
    private final Map<Integer, EnumPartsWithVariant> registered;
    public ItemMultiBotaniaParts(IPartHelper partHelper) {
        Preconditions.checkNotNull(partHelper);
        this.setUnlocalizedName("botaniaPart");
    ...
    @Nonnull
    public ItemStackSrc createPart(EnumPartsBotaniaTerminals mat) {
        Preconditions.checkNotNull(mat);
        return this.createPart(mat, 0);
    }

    @Nonnull
    public ItemStackSrc createPart(EnumPartsBotaniaTerminals mat, AEColor color) {
        Preconditions.checkNotNull(mat);
    ...
    @Nonnull
    private ItemStackSrc createPart(EnumPartsBotaniaTerminals mat, int varID) {
        assert mat != null;
    ...
    private void processMetaOverlap(boolean enabled, int partDamage, EnumPartsBotaniaTerminals mat, EnumPartsWithVariant pti) {
        assert partDamage >= 0;
    ...
    public int getDamageByType(EnumPartsBotaniaTerminals t) {
    ...
    public boolean onItemUse(ItemStack is, EntityPlayer player, World w, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        return this.getTypeByStack(is) == EnumPartsBotaniaTerminals.InvalidTypeBotania ? false : AEApi.instance().partHelper().placeBus(is, x, y, z, side, player, w);
    }
    ...
    public String getItemStackDisplayName(ItemStack is) {
        EnumPartsBotaniaTerminals pt = this.getTypeByStack(is);
        if (pt.isCable()) {
    ...
    private String getName(ItemStack is) {
        ...
        EnumPartsBotaniaTerminals stackType = this.getTypeByStack(is);
        String typeName = stackType.name();
        return typeName;
    }

    @Nonnull
    public EnumPartsBotaniaTerminals getTypeByStack(ItemStack is) {
        Preconditions.checkNotNull(is);
        EnumPartsWithVariant pt = (EnumPartsWithVariant)this.getRegistered().get(is.getItemDamage());
        return pt != null ? pt.part : EnumPartsBotaniaTerminals.InvalidTypeBotania;
    }

    @Nullable
    public IPart createPartFromItemStack(ItemStack is) {
        EnumPartsBotaniaTerminals type = this.getTypeByStack(is);
    ...
    protected static final class EnumPartsWithVariant {
        private final EnumPartsBotaniaTerminals part;
        private final int variant;
        @SideOnly(Side.CLIENT)
        private IIcon ico;

        private EnumPartsWithVariant(EnumPartsBotaniaTerminals part, int variant) {
            assert part != null;
