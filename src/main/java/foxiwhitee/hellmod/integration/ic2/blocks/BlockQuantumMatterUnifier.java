...
import foxiwhitee.hellmod.integration.ic2.client.gui.GuiQuantumMatterUnifier;
...
import foxiwhitee.hellmod.integration.ic2.tile.TileQuantumMatterUnifier;
...
@SimpleGuiHandler(index = GuiHandlers.quantumMatterUnifier, tile = TileQuantumMatterUnifier.class, container = ContainerQuantumMatterUnifier.class, gui = GuiQuantumMatterUnifier.class, integration = "IC2")
public class BlockQuantumMatterUnifier extends AEBaseTileBlock {
    protected final Random randy;

    public BlockQuantumMatterUnifier(String name) {
        super(Material.iron);
        this.randy = new Random();
        setHardness(10.0F);
        setBlockTextureName(HellCore.MODID + ":ae2/" + name);
        setBlockName(name);
        setTileEntity(TileQuantumMatterUnifier.class);
        this.isOpaque = false;
        this.lightOpacity = 1;
    }
    ...
    public boolean onActivated(World world, int x, int y, int z, EntityPlayer player, int p_149727_6_, float p_149727_7_, float p_149727_8_, float p_149727_9_) {
        ...
        if (tile instanceof TileQuantumMatterUnifier && !world.isRemote)
            player.openGui(HellCore.instance, GuiHandlers.quantumMatterUnifier, world, x, y, z);
        return true;
    }

    public void breakBlock(World world, int x, int y, int z, Block block, int wut) {
        TileQuantumMatterUnifier unifier = (TileQuantumMatterUnifier)world.getTileEntity(x, y, z);
        if (unifier != null) {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack itemstack) {
        TileEntity tile = world.getTileEntity(x, y, z);
        if (tile instanceof TileQuantumMatterUnifier) {
            TileQuantumMatterUnifier te = (TileQuantumMatterUnifier)tile;
