...
public class TileQuantumMatterUnifier extends AENetworkInvTile implements ISaveProvider, IGridTickable, IEUEnergyReceiver, IEnergyUpdateTile {
    ...
    private double maxEnergyStored = 100_000_000;

    private int matterGeneration = 0;

    private double needEnergyForOperation = 0.0D;

    public TileQuantumMatterUnifier() {
    ...
    @Override
    public double receiveEnergy(double paramDouble, Actionable paramActionable) {
        double d = maxEnergyStored - (double) energyStored;
        double can = Math.min(paramDouble, maxEnergyStored - (double) energyStored);
        if (can < 0D)
            return 0;
        if (paramActionable == Actionable.MODULATE)
            this.energyStored += can;
        if (Platform.isServer()) {
    ...
    public void setMaxEnergyStoredForUpdate(double paramDouble) {
        this.maxEnergyStored = paramDouble;
    }
