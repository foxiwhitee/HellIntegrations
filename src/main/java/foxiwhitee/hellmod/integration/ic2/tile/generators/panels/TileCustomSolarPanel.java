...
public abstract class TileCustomSolarPanel extends TileEntity implements IEnergyTile, IWrenchable, IEnergySource, IInventory, INetworkDataProvider, INetworkUpdateListener, INetworkTileEntityEventListener {
    ...
    public void updateEntity() {
        ...
        if (!super.worldObj.isRemote) {
            ...
            if (this.generating > 0) {
                if (this.storage + this.generating <= this.getMaxStorage()) {
                    this.storage += this.generating;
                } else {
                    this.storage = this.getMaxStorage();
                }
            }
    ...
    public int gainFuel() {
        ...
        if (this.sunIsUp && this.skyIsVisible) {
            this.generating = 0 + this.getGenDay();
            return this.generating;
        } else if (this.skyIsVisible) {
            this.generating = 0 + this.getGenNight();
            return this.generating;
        } else {
            this.generating = 0;
            return this.generating;
    ...
    public void updateVisibility() {
        Boolean rainWeather = this.wetBiome && (super.worldObj.isRaining() || super.worldObj.isThundering());
    ...
    public abstract int getGenNight();

    public abstract int getGenDay();
